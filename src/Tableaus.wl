...
TableauZerosQ[x_] := MatrixQ[x] && AllTrue[x, # === 0 &, 2];
...
TableauExplicitQ[x_] := MatrixQ[x] && x === LowerTriangularize[x, -1];
...
TableauDirkQ[x_] := MatrixQ[x] && x === LowerTriangularize[x];
...
TableauDiagonalQ[x_] := MatrixQ[x] && DiagonalMatrix[Diagonal[x]] === x
